...
function LoginPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setLoading(true);
            const res = await axios.post("https://agently-nfl2.onrender.com/login", formData);
        ...
        <div className="flex flex-col items-center justify-center py-20 px-10">
            <div className="w-full max-w-md bg-white rounded-lg">
                ...
                <div className="mb-8 text-center">
                    ...
                    <h1 className="mt-2 text-xl font-bold text-black">Welcome Back</h1>
                    <p className="text-black opacity-80">Log in to access Work</p>
                </div>
            </div>
            ...
            <form className="space-y-7 px-5" onSubmit={handleSubmit}>
                ...
                <button type="submit" disabled={loading} className="bg-[#FE6603] hover:bg-[#FF762D] font-semibold tracking-wide rounded-sm py-3 cursor-pointer text-white w-full">{ loading ? "Logging..." : "Log In"}</button>
            </form>
            ...
            <div className="text-center pt-5 pb-10">
                <p className="text-black/60">
                    ...
                    <Link className="text-[#FF6600] hover:text-[#FF6600]/90 px-2" to={"/createaccount"} data-discover="true">
                        Create an account.
                    </Link>
                </p>
            </div>
